...
import { CadetAchievements } from "./CadetAchievements";
import type { CampaignThemeConfig } from "@/types/campaignTheme";

export function CadetAchievementsWrapper() {
  ...
  useEffect(() => {
    async function loadCampaignTheme() {
      ...
      try {
        ...
        console.error("[CadetAchievementsWrapper] Failed to load theme:", error);
      } finally {
        setIsLoading(false);
      }
    }
